...
using LiteDB;
using MessagePack;
using RacingwayCore.Collision.Triggers;
...
public class Route
{
    ...
    [Key(0)] public byte[] IdBytes { get; set; }
    ...
    public Route(byte[] idBytes, string name, string author, string description, Address address, int laps, bool allowMounts)
    {
    ...
    public uint LastCheckpoint
    {
        get
        {
            var checkpoints = Triggers.Where(x => x is Checkpoint).Cast<Checkpoint>().ToArray();
